...
- PcBusinessPlanReopens (every reopen of an approved plan: user, circle, cycle, date) + SCBusinessPlanLogs (field-level edit log, sparse)
- JcWeeklyCalendars (the four weeks of every cycle - the planning windows are defined on them)
- ours, not mirrored: plan_snapshot, projection_snapshot, plan_approval_history (the plan's as-of history, see views_applied.md) and plan_name_alias (hand-kept name → item)
...
- **The approval ladder** (crm, Sep 2026): `1` open → `2` pending technical executive → `3` pending branch manager → `4` branch manager approved, pending regional / business head → `5` fully approved. `0` and `6` are never set by any procedure (legacy / an admin edit). **crm publishes to oracle from status 4 upward**, which is why `is_bm_approved` tracks the published projection and `is_approved` (status 5) does not: in 2025-26 JC5 the approved plan was 192k, the branch-approved plan 2.74M and the published projection 2.73M.
- **The windows**: a cycle's plan is entered during weeks 2 and 3 of the *previous* cycle (SE → TE → BM → RM/BH), compiled for the planners on the first day of week 4 and pushed to oracle on the cycle's last day. The dates come from `JcWeeklyCalendars` + `configurations` rows 114-122 / 388-389 and sit on `dim_jc` (`se_cutoff`, `te_auto_approval`, `bm_cutoff`, `bh_cutoff`, `handoff_date`, `publish_date`).
...
- `BiPoDetails.vendor_site_id` → **`ApSupplierSitesAlls.vendor_site_id`** on 100% of lines, and every site carries a country. That country is the best single predictor of an import lead time - far better than `procurement_type`, which lumps Spain and the United States together. Two codes are wrong at source and are folded in `dim_supplier_site`: every `KP` site is a South Korean firm and every `AS` site is American.
- **`BiStockDetail` is an opening balance.** A row dated X is the position at the *start* of X. Measured against vendor receipts, whose receipt date is real: 97% of their lots first appear on receipt date + 1, and only 0.1% on the day itself. Anything deriving an event date from the stock table must subtract a day - see `fact_internal_transfer`.
- **A lot number is a receipt batch id, not an item batch.** Thousands of lots appear under two or more items, one under several hundred. Any join on `lot_number` must carry `item_id` too, or one item inherits another's dates.
- **Payment terms resolve against `ApTermsTls`**, crm's mirror of the oracle terms master (87 rows, ids `10000` upward, with `due_days` and a term group code). It covers every term id we hold, in full: `ApSuppliers.terms_id`, `ApSupplierSitesAlls.terms_id`, `PurchaseRequisitionHdrs.payment_term_id` and `PurchaseRequisitionDtls.lastpotermid`. Do **not** reach for crm's `PaymentTerms` - that is the *customer receivables* master on a different id space (`4` / `5` / `1000+`) and matches none of these, which is what made it look unresolvable at first.
...
| 4 | SCBusinessPlanLogs | `line_id` | snapshot | – | blank header_id if the plan header is not loaded | – | – |
...
| 0 | JcWeeklyCalendars | `line_id` | upsert | – | – (no fks; `acc_yr` + `jcno` meet JourneyCalendars by name) | – | – |
| 2 | PcBusinessPlanReopens | `line_id` | snapshot | – | circle lower/trim + `unknown`; user → held back if not loaded | – | – |
